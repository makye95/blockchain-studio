...
build:
	...
	go build -o gitshock -ldflags="\
    	-X 'github.com/gitshock-labs/go-client/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/gitshock-labs/go-client/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/gitshock-labs/go-client/versioning.Branch=$(BRANCH)'\
		-X 'github.com/gitshock-labs/go-client/versioning.BuildTime=$(TIME)'" \
	main.go
...
test-e2e:
	...
	go build -race -o artifacts/gitshock .
	env EDGE_BINARY=${PWD}/artifacts/gitshock go test -v -timeout=30m ./e2e/...
